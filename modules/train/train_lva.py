...
class Train:
    ...
    def __init__(self, data_dir, obs_len, pred_len,
                 normalize_type, img_width, img_height, batch_size, validation_split=0.2,logger= None, results_logger= None):
        self._logger = logger
        ...
        self.train_data_dir=[os.path.join(data_dir,'train'), os.path.join(data_dir,'val')]
        self.test_data_dir=[os.path.join(data_dir,'test')]
        self.train_data= make_trajectories_array(self.train_data_dir,obs_len+pred_len,random_update=True)
        self.test_data= make_trajectories_array(self.test_data_dir,obs_len+pred_len)
    ...
    def train(self, epochs, save_path, early_stop=False, verbose_step=100, early_stop_patience=10, test_step=1):
        ...
        while epoch < epochs and not early_stop_flag:
            ...
            for phase in ['train', 'val']:
                ...
                for i, (batch_x, batch_y) in enumerate(data_loaders[phase]):
                    ...
                    if i%100==0:
                        print("Loss at epoch {}, batch {}/{} is {}".format(epoch,i,len(data_loaders[phase]),loss))
                    if phase == 'train':
                        loss.backward()
                        # update the weights
                        self.optimizer.step()
    ...
    def interval_test(self, epoch, losses, save_path):
        ...
        print("The results at epch {} are : \n norm_ade: {} \n norm_fde: {} \n ade:{}\n fde:{}".format(epoch,norm_ade,norm_fde,ade,fde))
